...
goods_list = []
'''获取店铺内产品销量信息'''
class PddAuthMallGoodsSalesV2Spider(scrapy.Spider):
	name = 'pdd_auth_mall_goods_sales_v2'

	alias_name = 'pdd_query_mall_home_info'

	'''待抓取产品销量的店铺列表'''
	mall_id_list 	= 'pdd_auth_mall_id_copy_list'
	list_name 	= ''
	hash_num 		= 0
	process_nums 	= 1
	limit			= 1000
	...
	def __init__(self, hash_num = 0, process_nums = 1):
		...
		self.list_name = self.mall_id_list
	...
	def save_mall_log(self, mall_id, mall_info):
		...
		file_path = '/data/spider/logs/auth_mall_log'

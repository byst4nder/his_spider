...
'''获取店铺内产品销量信息'''
class PddGoodsSalesCheckV5Spider(scrapy.Spider):
	name = 'pdd_goods_sales_check_v5'
	...
	def save_goods_log(self, goods_id, goods_info):
		...
		file_path = '/data/spider/logs/goods_sales_check_log'
		if not os.path.exists(file_path):
			os.makedirs(file_path)

		data = '[' + str(time.strftime('%Y-%m-%d %H:%M:%S')) + '] ' + goods_info

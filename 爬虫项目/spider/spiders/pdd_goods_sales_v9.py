...
goods_list = []
'''获取店铺内产品销量信息'''
class PddGoodsSalesV8Spider(scrapy.Spider):
	name = 'pdd_goods_sales_v9'
	...
	def start_requests(self):
		...
		while not is_end:
			...
			for mall_id in mall_ids:
				...
				meta = {'page':page, 'mall_id':mall_id, 'goods_list':goods_list, 'proxy':self.get_proxy_ip(False),'request_count': self.request_count+1}
	...
	def parse(self, response):
		...
		self.save_mall_success_log(mall_id, json.dumps({
	...
	def build_url(self, mall_id,page=1, page_size=500):
		...
		#最新接口  http://mobile.yangkeduo.com/proxy/api/api/turing/mall/query_mall_home_info?mall_id=170453&type=1&pdduid=0   
		return url
	...
	def get_ssdb_proxy_ip(self):
		ips = self.redis_client.hvals('proxy_ip_hash_2808')
		res = []
	...
	def save_mall_success_log(self, mall_id, success_info):
		date = time.strftime('%Y-%m-%d')

		file_path = '/data/spider/log/mall_success_log'
		if not os.path.exists(file_path):
			os.makedirs(file_path)

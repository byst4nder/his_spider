...
'''获取店铺信息及销量记录'''
class PddGoodsSkuV3Spider(scrapy.Spider):
	name = 'pdd_goods_sku_v3'
	mall_id_list = 'pdd_sync_sku_goods_id_list'  # 复制店铺队列（不使用hash！）
	hash_num 		= 0
	process_nums 	= 1
	limit			= 500
	...
	def start_requests(self):
		...
		while not is_end:
			...
			mall_ids = self.ssdb.qpop_front(self.mall_id_list, 1)
			...
			for mall_id in mall_ids:
				...
				yield scrapy.Request('http://mobile.yangkeduo.com/goods.html?goods_id='+str(mall_id), meta=meta,callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
			time.sleep(1)

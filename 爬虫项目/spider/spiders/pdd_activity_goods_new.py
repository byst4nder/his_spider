...
class PddActivityGoodsNewSpider(scrapy.Spider):
	name  = 'pdd_activity_goods_new'
	custom_settings = {
	...
	def start_requests(self):
		...
		hash_names = ['pdd_goods_activity_1', 'pdd_goods_activity_2']
				...
				for subject_info in subject_data:
					...
					if type(subject_info) == dict:
						...
						if subject_info['type'] == 1:
							...
							yield scrapy.Request(url, meta=meta, callback=self.parse_subject_info)
						...
						if goods_info['jump']:
							for goods_link in goods_info['jump']:
								...
								if goods_id:
									...
									yield scrapy.Request(url, meta={'goods_info':goods_info}, callback=self.parse_goods_info)
	'''获取产品信息'''

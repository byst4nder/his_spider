...
"""
import scrapy
import json, time, random, pyssdb, pddSign, urllib,setting, logging, redis, datetime
from spider.items import KeywordGoodsList
from scrapy.utils.project import get_project_settings

class PddKeywordGoodsV8RankSpider(scrapy.Spider):
	handle_httpstatus_list = [403]
	FEED_EXPORT_ENCODING = 'utf-8'
	name = 'pdd_keyword_goods_v8_rank'
	...
	def start_requests(self):
		...
		while search:
			...
			for hot_keyword in hot_keywords:
				...
				meta = {'flip': flip, 'page': page, 'keyword': hot_keyword, 'sort': sort, 'p_time': detail['p_time']}
	...
	def parse(self, response):
		...
		if len(item_info) > 0:
			...
			if page <= self.max_page:
				...
				meta = {'flip': flip, 'proxy': proxy, 'page': page, 'keyword': keyword, 'sort': sort, 'p_time': p_time}
				yield scrapy.Request(url, meta=meta, callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
	...
	def err_after(self, meta):
		...
		if page > 6:
			self.ssdb_client.qpush_back(self.retry_list_name, json.dumps({'flip': flip, 'page': page, 'keyword': keyword, 'sort': sort, 'p_time': p_time}))  # 失败关键词重新放入队列
		else:
			# 第一页失败直接推回队列
			self.ssdb_client.qpush_back(self.list_name, json.dumps({'flip': flip, 'page': page, 'keyword': keyword, 'sort': sort, 'p_time': p_time}))  # 失败关键词重新放入队列

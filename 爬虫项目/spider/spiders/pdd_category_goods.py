...
# 获取首页分类下的产品列表
import scrapy
import json, time, sys, random, pddSign, urllib,setting, pyssdb,string
...
class PddCategoryGoodsSpider(scrapy.Spider):
	name  = 'pdd_category_goods'
	...
	def parse(self, response):
		...
		if data['goods_list']:
			...
			for goods_data in data['goods_list']:
				...
				goods_lists.append({'goods_id': goods_id, 'rank': rank, 'subject_id': cat_id, 'type': 2, 'price': price})

...
"""
import scrapy
import json, time, random, pyssdb, pddSign, urllib, redis,setting, logging
from spider.items import KeywordGoodsList
from scrapy.utils.project import get_project_settings

class PddKeywordGoodsV5Spider(scrapy.Spider):
	handle_httpstatus_list = [403]
	FEED_EXPORT_ENCODING = 'utf-8'
	name = 'pdd_keyword_goods_v5'
	...
	def start_requests(self):
		...
		while search:
			...
			for hot_keyword in hot_keywords:
				...
				page = 1
				meta = {'proxy': self.get_proxy_ip(False), 'page': page, 'keyword': hot_keyword, 'sort': 0}
	...
	def err_after(self, meta):
		...
		if page > 1:
			self.ssdb_client.qpush_back(self.retry_list_name, json.dumps({'page': page, 'keyword': keyword, 'sort': sort, 'p_time': self.p_time}))  # 失败关键词重新放入队列
		else:
			# 第一页失败直接推回队列

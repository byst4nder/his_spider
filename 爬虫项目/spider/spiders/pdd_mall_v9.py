...
'''获取店铺信息及销量记录'''
class PddMallV9Spider(scrapy.Spider):
	name = 'pdd_mall_v9'
	...
	def get_proxy_ip(self, refresh):
		if not refresh and self.proxy_count < 1000:
			...
			if len(self.proxy_ip_list) <= 0:
				self.ssdb.set('16y_much_proxy_flag', 1)
				# print('no ip')
				time.sleep(1)
				self.proxy_ip_list = self.get_ssdb_proxy_ip()
				# print('no ip 2', self.proxy_ip_list)
				return self.get_proxy_ip(refresh)

			# print(len(self.proxy_ip_list))
	...
	def get_ssdb_proxy_ip(self):
		ips = self.ssdb.hkeys('proxy_ip_hash_v9', '', '', 1000)
		res = []

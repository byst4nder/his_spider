...
'''获取店铺信息及销量记录'''
class PddMallV10Spider(scrapy.Spider):
	name = 'pdd_mall_v10'
	...
	def start_requests(self):
		...
		while not is_end:
			...
			for mall_id in mall_ids:
				...
				yield scrapy.Request('http://mobile.yangkeduo.com/mall_page.html?mall_id='+str(mall_id), meta=meta,callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
			time.sleep(1)
	...
	def make_headers(self):
		...
		headers = {
			...
			# "Referer":"Android",
			# "Connection":"keep-alive",
			"Cookie": '_nano_fp=XpdYX0gJlpgon0dxl9_Vfj_vkXR1TNT9PuWUpMqq; api_uid=' + str( self.ssdb_client.get('pdd_api_uid') ),

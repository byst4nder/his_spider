...
class PddKeywordGoodsV8RetrySpider(scrapy.Spider):
	handle_httpstatus_list = [403]
	FEED_EXPORT_ENCODING = 'utf-8'
	name = 'pdd_keyword_goods_v8_retry'
	...
	def parse(self, response):
		...
		if len(item_info) > 0:
			...
			if page <= self.max_page:
				...
				self.ssdb_client.qpush_back(self.rank_list_name, json.dumps({'page': page, 'keyword': keyword, 'sort': sort, 'p_time': p_time}))  # 失败关键词重新放入队列

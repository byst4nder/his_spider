...
class PddMallNewSpider(scrapy.Spider):
	...
	def start_requests(self):
		...
		while not is_end:
			...
			mall_ids = self.ssdb.qpop_front(self.mall_id_list, 240)
			...
			for mall_id in mall_ids:
				...
				yield scrapy.Request('http://apiv3.yangkeduo.com/mall/'+str(mall_id)+'/info?query_mall_favorite_coupon=true&timestamp='+str(int(time.time()))+str(random.randint(100,999))+'&coupon_batch_version=1&pdduid=0', meta=meta,callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
			time.sleep(1)
	...
	def get_proxy_ip(self, refresh):
		if not refresh and self.proxy_count < 20:

...
class PddMallKeywordsV6Spider(scrapy.Spider):
    name = 'pdd_promotion_keywords_v6'
    laravel_cookie = ''
    ...
    def start_requests(self):
        ...
        for cat_id_1 in cat_1_list:
            ...
            yield scrapy.Request("https://mms.pinduoduo.com/venus/api/subway/analysis/queryCategoryInfo",
                ...
                }), meta={"mallId": mall_id, "cookie": cookie, "passId": pass_id, "cat_id_1": cat_id_1, 'level': 1,}, headers={
                    'Content-Type': 'application/json',
                    'Cookie': cookie,
                    'Referer': 'https://mms.pinduoduo.com/exp/tools/dataAnalysis',
                }, callback=self.parse_category_info, errback=self.errback_httpbin)
    ...
    def parse_category_info(self, response):
        ...
        for child_category in mall_list['catInfoList']:
            ...
            yield scrapy.Request("https://mms.pinduoduo.com/venus/api/subway/analysis/queryCategoryInfo",
                    ...
                    'Cookie': cookie,
                    'Referer': 'https://mms.pinduoduo.com/exp/tools/dataAnalysis',
                }, callback=self.parse_third_category, errback=self.errback_httpbin)
    ...
    def errback_httpbin(self, failure):
        ...
        if failure.check(HttpError):
            ...
            self.err_after(request, True)
        else:
            errmsg = 'errback <%s> OtherError' % request.url
            self.err_after(request.meta, False)
        self.save_error_log(json.dumps({
            'meta': request.meta,
            'errmsg': errmsg,
        }))

    def err_after(self, request, state):
        pass

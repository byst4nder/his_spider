...
'''获取店铺信息及销量记录'''
class PddMallV15Spider(scrapy.Spider):
	name = 'pdd_mall_v15'
	...
	def start_requests(self):
		...
		while not is_end:
			# mall_ids 	=	self.ssdb_client.hkeys(self.mall_id_hash, self.start_mall_id, '', mall_nums)
			...
			for mall_id in mall_ids:
				...
				yield scrapy.Request('http://mobile.yangkeduo.com/proxy/api/mall/'+str(mall_id)+'/info?query_mall_favorite_coupon=true&coupon_batch_version=1&pdduid=0', meta=meta,callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
			time.sleep(1)
	...
	def parse(self, response):
		...
		logging.debug(json.dumps(mall_info))
	...
	def get_proxy_ip(self, refresh):
		if self.current_proxy != '' and not refresh and self.proxy_count < 100:
			...
			if len(self.proxy_ip_list) <= 0:
				return ''
			# logging.debug('proxy_count', len(self.proxy_ip_list))
		...
		# logging.debug(ip, self.proxy_count)

...
class PddMallKeywordsV10Spider(scrapy.Spider):
    name = 'pdd_promotion_keywords_v10'
    laravel_cookie = ''
            ...
            for day in self.days:
                ...
                for rankType in self.rankTypes:
                    ...
                    yield scrapy.Request(self.url, method="POST", body=json.dumps(query_data), meta=meta,
                                         headers=headers, callback=self.parse, dont_filter=False, errback=self.errback_httpbin)
            ...
            if request.url == self.url:
                ...
                if level == 1:
                    yield scrapy.Request(request.url, method="POST", body=body, meta=meta,
                                         ...
                                         dont_filter=False, errback=self.errback_httpbin)
                elif level == 2:
                    yield scrapy.Request(request.url, method="POST", body=body, meta=meta,

...
# 获取首页分类下的产品列表
import scrapy
import json, time, sys, random, pddSign, urllib,setting, pyssdb,string,os
...
class PddCategoryGoodsV4Spider(scrapy.Spider):
	name = 'pdd_category_goods_v4'
	...
	def start_requests(self):
		...
		while not is_end:
			...
			if type(detail) != int: ##当值是subject_id 时
				...
				meta = {'cat_id':cat_id,'opt_type':opt_type,'offset':offset, 'category':category}
				# print(url)
	...
	def parse(self, response):
		...
		goods_lists = []
		# print('parse_before', offset, cat_id)

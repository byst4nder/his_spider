...
class PddMallCheck(scrapy.Spider):
    name = 'pdd_mall_check'
    queue_name = 'pdd_mall_check_list'
    proxy_start_time = 0
    proxy_ip_list = []
    ssdb = ''
    ...
    def start_requests(self):
        ...
        while search:
            ...
            mall_ids = self.ssdb.qpop_front(self.queue_name, 1)  # 获取队列中的数据
            if mall_ids == True or mall_ids == None:
                time.sleep(30)
                continue

            if type(mall_ids) == bytes:  # 只有一条数据时，返回的是bytes格式，转为list
                mall_ids = [mall_ids]
            ...
            for mall_id in mall_ids:
                ...
                yield scrapy.Request('http://apiv4.yangkeduo.com/mall/'+str(mall_id)+'/info', meta=meta, callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)
    ...
    def make_headers(self):
        ...
        headers = {
            ...
            # 'AccessToken':'52LJMDSE26XBRYJ46B3NQBY4LB2VTD3GVVHEOWRNUJPQIXNYFEWA1005fa9',
        }

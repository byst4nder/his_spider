...
class PddKeywordGoodsRankV2Spider(scrapy.Spider):
	handle_httpstatus_list = [403]
	FEED_EXPORT_ENCODING = 'utf-8'
	name = 'pdd_keyword_goods_rank_v2'
	list_name = 'pdd_keyword_use_for_goods_rank_list_v2'  # 热搜词SSDB队列名每天需要爬的数据
	err_list_name = 'pdd_keyword_use_for_goods_rank_list_error'  # 热搜词SSDB失败队列
	...
	def errback_httpbin(self, failure):
		...
		if page > 1:
			self.ssdb.qpush_back(self.err_list_name, json.dumps({'page': page, 'keyword': keyword, 'sort': sort}))  # 失败关键词重新放入队列
		else:
			# 第一页失败直接推回队列

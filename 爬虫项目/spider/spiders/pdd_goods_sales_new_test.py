# -*- coding: utf-8 -*-
import scrapy
import json, time, sys, random, re, pyssdb, pddSign, os, datetime, setting
...
goods_list = []
'''获取店铺内产品销量信息'''
class PddGoodsSalesNewTestSpider(scrapy.Spider):
	name = 'pdd_goods_sales_new_test'
	...
	def __init__(self, hash_num = 0, process_nums = 1):
		...
		self.ssdb 	= pyssdb.Client(get_project_settings().get('SSDB_HOST'), 8888)
	...
	def start_requests(self):
		...
		while not is_end:
			mall_id 	=	self.ssdb.qpop_front(self.list_name, 1)
			mall_id     = int( mall_id.decode('utf-8'))

			print(mall_id)

			self.ssdb.qpush_back(self.list_name, str(mall_id))
			goods_list=[]
			page = 1
			...
			meta = {'page':page, 'mall_id':mall_id, 'goods_list':goods_list, 'proxy':self.get_proxy_ip()}
	...
	def build_url(self, mall_id,page=1, page_size=500):
		...
		#url = 'http://apiv3.yangkeduo.com/api/turing/mall/query_cat_goods?category_id=0&type=0&mall_id='+str(mall_id)+'&page_no='+str(page)+'&page_size='+str(page_size)+'&sort_type=_sales&pdduid='+str(self.get_uid())
		# url = 'http://apiv4.yangkeduo.com/api/turing/mall/query_mall_home_info?mall_id='+str(mall_id)+'&pdduid=&type=1'
		url = 'https://api.yangkeduo.com/api/turing/mall/query_mall_home_info?mall_id='+str(mall_id)+'&type=1&pdduid=&page_from=39'
		return url
	...
	def get_proxy_ip(self):
		...
		while not is_end:
			...
			# print(proxy)
			if proxy['last_used_at'] < now_time - 3:
				is_end = True
				ip = proxy['proxy']
	...
	def errback_httpbin(self, failure):
		...
		# self.ssdb.qpush_back(self.list_name, mall_id)  # 失败关键词重新放入队列

...
'''获取店铺信息及销量记录'''
class PddMallProxy16ySpider(scrapy.Spider):
	name = 'pdd_mall_proxy_16y'
	...
	custom_settings = {
		'DOWNLOADER_MIDDLEWARES': {'spider.middlewares.Proxy16yMiddleware': 543},
		# 'LOG_FILE':'',
		# 'LOG_LEVEL':'DEBUG',
		# 'LOG_ENABLED':True,
	...
	def start_requests(self):
		...
		while not is_end:
			...
			for mall_id in mall_ids:
				...
				yield scrapy.Request('http://api.yangkeduo.com/mall/'+str(mall_id)+'/info?query_mall_favorite_coupon=true', meta=meta,callback=self.parse, headers=headers, dont_filter=True, errback=self.errback_httpbin)

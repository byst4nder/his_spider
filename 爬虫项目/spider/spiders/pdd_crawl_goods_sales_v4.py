...
class PddCrawlGoodsSalesSpider(scrapy.Spider):
	name = 'pdd_crawl_goods_sales_v4'
	alias_name = 'get_pdd_goods_info'
	goods_list = 'pdd_crawl_goods_id_list:'
	hash_num = 0
	process_nums = 1
	...
	def errback_httpbin(self, failure):
		self.error_count += 1
		# self.save_count_log('error', json.dumps({
	...
	def save_count_log(self, log_type, info):
		date = time.strftime('%Y-%m-%d')

		file_path = '/data/spider/logs/'+str(log_type)+'_log/'+str(self.name)+'/'+str(self.hash_num)
		if not os.path.exists(file_path):
			os.makedirs(file_path)

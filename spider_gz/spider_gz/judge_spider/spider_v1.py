...
class JudgeSpier:
    ...
    def get_sougou_title(self, words):
        items = []
        items3 = []
        try:
            resp = requests.get(url=self.sougou_url.format(words), headers=self.sougou_headers, timeout=5)
            time.sleep(random.choice([0.8, 0.5, 0.3, 0.1, 0.2, 0.11, 0.16]))
            html_encode = resp.content
            detect_result = chardet.detect(html_encode)
            encoding = detect_result["encoding"]
            html = str(html_encode, encoding)
            soup = BeautifulSoup(html, 'html.parser')
            soup1 = soup.find_all('div', class_='results')
            if soup1:
                items1 = soup1[0].select('h3 a')
                a = 0
                for i in items1:
                    title = i.text.strip()
                    if title.__contains__(words):
                ...
                items2 = soup1[0].select("div cite")
                for i in items2:
                    items3.append(i.text.strip())
    ...
    def request_sougou(self, words):
        title_list, source_list = self.get_sougou_title(words)
        if title_list:
            a = 0
            tag = False
        ...
        for file in file_path_list:
            ...
            while True:
                ...
                if self.request_baidu(word):
                    print("百度搜索不符合要求词汇:", word)
                    self.save_del_word("百度搜索不符合要求词汇：" + word)
                    if self.request_sougou(word):
                        print("搜狗搜索不符合要求词汇:", word)
                        self.save_del_word("搜狗搜索不符合要求词汇：" + word)
                        continue
                    print("搜狗搜索符合要求词汇:", word)
